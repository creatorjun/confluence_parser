...
def _inline(para, node):
    if isinstance(node, NavigableString):
        t = str(node)
        if t.strip(): para.add_run(t)
        return
    tag = node.name
    if tag in ("strong","b"):
        para.add_run(node.get_text()).bold = True
    elif tag in ("em","i"):
        r = para.add_run(node.get_text()); r.italic = True
    elif tag == "code":
        ...
        for c in node.children: _inline(para, c)
...
def _code_block(doc: Document, text: str):
    ...
    s = OxmlElement("w:shd")
    s.set(qn("w:val"),"clear"); s.set(qn("w:color"),"auto"); s.set(qn("w:fill"),"F4F4F4")
    pPr.append(s)
    pb = OxmlElement("w:pBdr")
    lft = OxmlElement("w:left")
    lft.set(qn("w:val"),"single"); lft.set(qn("w:sz"),"12")
    lft.set(qn("w:space"),"4"); lft.set(qn("w:color"),"4A90D9")
...
def _table(doc: Document, tbl: Tag):
    ...
    for ri, row in enumerate(rows):
        ...
        for ct in row.find_all(["td","th"]):
            ...
            cell = t.cell(ri, ci)
            if ct.name == "th": _shd(cell, "E8F0FE")
            _border(cell)
            para = cell.paragraphs[0]; para.clear()
            for ch in ct.children: _inline(para, ch)
            for r in para.runs:
                r.font.size = Pt(9)
                if ct.name == "th": r.bold = True
...
def _block(doc: Document, node):
    ...
    if tag in ("h1","h2","h3","h4","h5","h6"):
        p = doc.add_paragraph(style=_HM[int(tag[1])]); p.clear()
        for c in node.children: _inline(p, c)
    elif tag == "p":
        ...
        if any(c in classes for c in ("code","codeContent","syntaxhighlighter","highlight")):
            ...
            for c in node.children: _block(doc, c)
    elif tag == "table":
        if is_code_table(node): _code_block(doc, extract_code_lines(node))
        else: _table(doc, node)
        doc.add_paragraph()
    elif tag in ("ul","ol"):
        style = "List Bullet" if tag == "ul" else "List Number"
        for li in node.find_all("li", recursive=False):
            p = doc.add_paragraph(style=style)
            for c in li.children: _inline(p, c)
    elif tag == "hr":
...
def convert(pages: list[tuple[int,str,str]], output_path: str) -> None:
    ...
    for depth, title, html_body in pages:
        doc.add_paragraph(title, style=HM[min(depth+1,4)])
        soup = clean(BeautifulSoup(html_body, "html.parser"))
        for c in soup.children: _block(doc, c)
        if depth == 0: doc.add_page_break()
